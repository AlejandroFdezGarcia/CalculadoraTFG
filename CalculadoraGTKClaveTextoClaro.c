...
void on_key_enter(GtkButton *button, gpointer data) {
    ...
    if (g_strcmp0(key, "ABC123") == 0 || g_strcmp0(key, "DEF456") == 0) {
        gtk_widget_hide(gtk_widget_get_toplevel(key_entry));
...
void create_key_entry_window() {
    ...
    gtk_window_set_title(GTK_WINDOW(key_window), "Verificacion de clave");
    gtk_window_set_default_size(GTK_WINDOW(key_window), 300, 100);
